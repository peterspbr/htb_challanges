...
class TimeCapsule():
    ...
    def _get_new_pubkey(self):
        while True:
            ...
            try:
                ...
                pow(self.e, -1, phi) # d = e^-1 mod phi
                break
            except ValueError:
                pass
    ...
    def get_new_time_capsule(self):
        n, e = self._get_new_pubkey() # Generate the public (e) and private (n) keys
        m = bytes_to_long(self.msg) # gets the message
        # Encrypts the message
        m = pow(m, e, n) # m^e % n

        # Return the encrypted message, the private and public key in hexadecimal.
